...
class CustomFormExampleFixture extends Fixture
{
    public function load(ObjectManager $manager): void
    {
        ...
        $statuses = [
            CustomFormExample::STATUS_NEW,
            CustomFormExample::STATUS_PENDING,
            CustomFormExample::STATUS_APPROVED,
            CustomFormExample::STATUS_REJECTED,
        ];

        for ($i = 0; $i < 50; $i++) {
            $entity = new CustomFormExample();
            $entity->setTitle($generator->sentence(3));
            $entity->setStatus($generator->randomElement($statuses));

            // Use one of the real demonstration images
            if ($i % 3 === 0) { // Every 3rd entity gets an image
                $entity->setFile($generator->randomElement($binArray));
            }
